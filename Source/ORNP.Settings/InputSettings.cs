...
namespace ORNP.Settings
{
    ...
    {
        static ICatalog commonCatalog = new Catalog("ORNP.Common", "./locale");
        static ICatalog settingsCatalog = new Catalog("ORNP.Settings", "./locale");
            ...
            foreach (var command in GetCommands())
            {
                ...
                if (modInput != null)
                {
                    if (modInput.Shift && modInput.IgnoreShift)
                        errors.Add(settingsCatalog.GetString("{0} requires and is modified by Shift", GetPrettyLocalizedName(command)));
                    if (modInput.Control && modInput.IgnoreControl)
                        errors.Add(settingsCatalog.GetString("{0} requires and is modified by Control", GetPrettyLocalizedName(command)));
                    if (modInput.Alt && modInput.IgnoreAlt)
                        errors.Add(settingsCatalog.GetString("{0} requires and is modified by Alt", GetPrettyLocalizedName(command)));
                }
            }
            ...
            for (var command1 = firstCommand; command1 <= lastCommand; command1++)
            {
                ...
                for (var command2 = command1 + 1; command2 <= lastCommand; command2++)
                {
                    ...
                    foreach (var uniqueInput in sharedUnique)
                        errors.Add(settingsCatalog.GetString("{0} and {1} both match {2}", GetPrettyLocalizedName(command1), GetPrettyLocalizedName(command2), GetPrettyUniqueInput(uniqueInput)));
                }
            }
