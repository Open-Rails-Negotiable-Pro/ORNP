...
namespace Ornp.Formats.Msts
{
    ...
    public class TempSpeedPostItem : SpeedPostItem
    {
        ...
        public TempSpeedPostItem(Tr_RouteFile routeFile, Position position, bool isStart, WorldPosition worldPosition, bool isWarning)
        {
            ...
            if (speed < 0) speed = ORNP.Common.MpS.FromKpH(25); //todo. Value is not used. Should it be used below instead of TempRestrictedSpeed? And if so, is the +0.01 then still needed?
            if (routeFile.MilepostUnitsMetric == true)
            {
                this.IsMPH = false;
                SpeedInd = (int)(ORNP.Common.MpS.ToKpH(routeFile.TempRestrictedSpeed) + 0.1f);
            }
            else
            {
                this.IsMPH = true;
                SpeedInd = (int)(ORNP.Common.MpS.ToMpH(routeFile.TempRestrictedSpeed) + 0.1f);
            }
