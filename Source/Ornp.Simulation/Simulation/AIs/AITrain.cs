            ...
            for (int iWait = 0; iWait <= TCRoute.WaitingPoints.Count - 1; iWait++)
            {
                ...
                if (!IsActualPlayerTrain)
                {
                    if (waitingPoint[2] >= 60011 && waitingPoint[2] <= 60021)
                    {
                        ...
                        {
                            case 11:
                                hornPattern = AILevelCrossingHornPattern.CreateInstance(ORNP.Common.LevelCrossingHornPattern.US);
                                break;
                            default:
                                hornPattern = AILevelCrossingHornPattern.CreateInstance(ORNP.Common.LevelCrossingHornPattern.Single);
                                break;
                        }
