...
namespace Ornp.Simulation.RollingStocks.SubSystems
{
...
#pragma warning restore CS0618 // SetEmergency is obsolete
        ...
        public void Initialize()
        {
            if (!Activated)
            {
                ...
                if (SoundFileName != null)
                {
                    ...
                    var soundPath = ORNPPaths.GetFileFromFolders(soundPathArray, SoundFileName);
        ...
        private Aspect NextNormalSignalDistanceHeadsAspect()
        {
            ...
            if (signal != null)
            {
                foreach (var signalHead in signal.SignalHeads)
                {
                    ...
                    {
                        return distanceSignalAspect = (Aspect)Locomotive.Train.signalRef.TranslateToTCSAspect(signal.this_sig_lr(Ornp.Formats.Msts.MstsSignalFunction.DISTANCE));
                    }
                }
            }
        ...
        private T LoadParameter<T>(string sectionName, string keyName, T defaultValue)
        {
            ...
            var length = ORNP.Common.NativeMethods.GetPrivateProfileString(sectionName, keyName, null, buffer, buffer.Length, ParametersFileName);
            if (length > 0)
            {
                buffer.Trim();
