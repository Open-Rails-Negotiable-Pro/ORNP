...
using Ornp.Simulation.Timetables;
using ORNP.Common;
using System;
...
namespace Ornp.Simulation.Signalling
{
            ...
            foreach (TrackNode thisNode in trackDB.TrackNodes)
            {
                if (thisNode != null && thisNode.TrVectorNode != null)
                {
                    ...
                    foreach (TrVectorSection thisSection in thisVNode.TrVectorSections)
                    {
                        ...
                        float thisLength = 0f;
                        Ornp.Formats.Msts.TrackSection TS = tsectiondat.TrackSections[thisSection.SectionIndex];
    ...
    public class SignalWorldObject
    {
        ...
        public SignalWorldObject(Ornp.Formats.Msts.SignalObj SignalWorldItem, SignalConfigurationFile sigcfg)
        {
            Ornp.Formats.Msts.SignalShape thisCFGShape;

            HeadReference = new Dictionary<uint, uint>();

            // set flags with length to number of possible SubObjects type

            FlagsSet = new bool[Ornp.Formats.Msts.SignalShape.SignalSubObj.SignalSubTypes.Count];
            FlagsSetBackfacing = new bool[Ornp.Formats.Msts.SignalShape.SignalSubObj.SignalSubTypes.Count];
            ...
            if (sigcfg.SignalShapes.TryGetValue(SFileName, out thisCFGShape))
            {
                ...
                for (int iHead = 0; iHead < thisCFGShape.SignalSubObjs.Count; iHead++)
                {
                    ...
                    Ornp.Formats.Msts.SignalShape.SignalSubObj thisSubObjs = thisCFGShape.SignalSubObjs[iHead];
                    if (headSet != 0)
                    {
                ...
                foreach (Ornp.Formats.Msts.SignalUnit signalUnitInfo in SignalWorldItem.SignalUnits.Units)
                {
