    ...
    public class HelpWindow : Window
    {
            ...
            : base(owner, Window.DecorationSize.X + owner.TextFontDefault.Height * 37, Window.DecorationSize.Y + owner.TextFontDefault.Height * 24, Viewer.Catalog.GetString("Help"))
        {
            ...
            if (owner.Viewer.Simulator.Activity != null)
            {
                ...
                Tabs.Add(new TabData(Tab.ActivityWorkOrders, Viewer.Catalog.GetString("Work Orders"), (cl) =>
                {
                    ...
                    if (owner.Viewer.Simulator.ActivityRun.EventList != null)
                    {
                        ...
                        {
                            var eventAction = @event.ParsedObject as Ornp.Formats.Msts.EventCategoryAction;
                            if (eventAction != null)
                            {
                                if (separatorShown)
                                ...
                                {
                                    case Ornp.Formats.Msts.EventType.AssembleTrain:
                                    case Ornp.Formats.Msts.EventType.AssembleTrainAtLocation:
                                        line.Add(new Label(colWidth * 4, line.RemainingHeight, Viewer.Catalog.GetString("Assemble Train")));
                                        if (eventAction.Type == Ornp.Formats.Msts.EventType.AssembleTrainAtLocation)
                                        {
                                        ...
                                        break;
                                    case Ornp.Formats.Msts.EventType.DropOffWagonsAtLocation:
                                        line.Add(new Label(colWidth * 4, line.RemainingHeight, Viewer.Catalog.GetString("Drop Off")));
                                        break;
                                    case Ornp.Formats.Msts.EventType.PickUpPassengers:
                                    case Ornp.Formats.Msts.EventType.PickUpWagons:
                                ...
                                if (eventAction.WagonList != null)
                                {
                                    ...
                                    var wagonIdx = 0;
                                    var locationFirst = "";
                                    foreach (Ornp.Formats.Msts.WorkOrderWagon wagonItem in eventAction.WagonList.WorkOrderWagonList)
                                    {
                                        if (locationShown)
                                        {
                                            line = scrollbox.AddLayoutHorizontalLineOfText();
                                        ...
                                        {
                                            foreach (Ornp.Formats.Msts.ActivityObject activityObject in owner.Viewer.Simulator.Activity.Tr_Activity.Tr_Activity_File.ActivityObjects.ActivityObjectList)
                                            {
                                                if (activityObject.ID == trainIndex)
                                                {
                                                    foreach (Ornp.Formats.Msts.Wagon trainWagon in activityObject.Train_Config.TrainCfg.WagonList)
                                                    {
                                        ...
                                        // Location column
                                        if (locationShown &&
                                            !((eventAction.Type == Ornp.Formats.Msts.EventType.PickUpPassengers) || (eventAction.Type == Ornp.Formats.Msts.EventType.PickUpWagons)))
                                        {
                                            line.AddSpace(colWidth * 7, 0);
                                        }
                                        else
                                        {
                                            var sidingId = eventAction.Type == Ornp.Formats.Msts.EventType.AssembleTrainAtLocation
                                                || eventAction.Type == Ornp.Formats.Msts.EventType.DropOffWagonsAtLocation
                                                ? (uint)eventAction.SidingId : wagonItem.SidingId;
                                            foreach (var item in owner.Viewer.Simulator.TDB.TrackDB.TrItemTable)
                                            {
                                                var siding = item as Ornp.Formats.Msts.SidingItem;
                                            ...
                                            else if (location == "" | (eventAction.Type == Ornp.Formats.Msts.EventType.PickUpPassengers) || (eventAction.Type == Ornp.Formats.Msts.EventType.PickUpWagons))
                                                line.AddSpace(colWidth * 7, 0);
            ...
            if (owner.Viewer.Simulator.TimetableMode)
            {
                ...
                {
                    var tTTrain = owner.Viewer.SelectedTrain as Ornp.Simulation.Timetables.TTTrain;
                    var scrollbox = cl.AddLayoutScrollboxVertical(cl.RemainingWidth);
