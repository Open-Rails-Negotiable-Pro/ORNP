    ...
    public class MultiPlayerWindow : Window
    {
        ...
        public IEnumerable<ListLabel> MultiPlayerWindowList(Viewer viewer)
        {
            ...
            if (Ornp.MultiPlayer.MPManager.IsMultiPlayer())
            {
                ...
                var multiPlayerStatus = Ornp.MultiPlayer.MPManager.IsServer()
                    ? $"{Viewer.Catalog.GetString("Dispatcher")} ({Ornp.MultiPlayer.MPManager.Server.UserName})" : Ornp.MultiPlayer.MPManager.Instance().AmAider
                    ? Viewer.Catalog.GetString("Helper") : Ornp.MultiPlayer.MPManager.IsClient()
                    ? $"{Viewer.Catalog.GetString("Client")} ({Ornp.MultiPlayer.MPManager.Client.UserName})" : "";
            ...
            else if (Ornp.MultiPlayer.MPManager.Simulator.Confirmer != null)
            {
