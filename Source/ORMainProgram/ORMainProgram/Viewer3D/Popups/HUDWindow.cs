...
using Ornp.Simulation.AIs;
using Ornp.Simulation.Physics;
using Ornp.Simulation.RollingStocks;
using Ornp.Simulation.RollingStocks.SubSystems.Brakes;
using Ornp.Simulation.RollingStocks.SubSystems.Brakes.MSTS;
using Ornp.Simulation.RollingStocks.SubSystems.PowerSupplies;
using Ornp.Viewer3D.Processes;
using ORNP.Common;
...
namespace Ornp.Viewer3D.Popups
{
    public class HUDWindow : LayeredWindow
    {
        ...
        void TextPageCommon(TableData table)
        {
            ...
            if (Ornp.MultiPlayer.MPManager.IsClient())
                TableAddLabelValue(table, Viewer.Catalog.GetString("Time"), FormatStrings.FormatTime(Viewer.Simulator.ClockTime + Ornp.MultiPlayer.MPManager.Instance().serverTimeDifference));
            else
            ...
            {
                var text = Ornp.MultiPlayer.MPManager.Instance().GetOnlineUsersInfo();

                TableAddLabelValue(table, Viewer.Catalog.GetString("MultiPlayerStatus: "), "{0}", Ornp.MultiPlayer.MPManager.IsServer()
                    ? Viewer.Catalog.GetString("Dispatcher") : Ornp.MultiPlayer.MPManager.Instance().AmAider
                    ? Viewer.Catalog.GetString("Helper") : Ornp.MultiPlayer.MPManager.IsClient()
        ...
        void TextPageForceInfo(TableData table)
        {
            ...
            if (mstsLocomotive != null)
            {
                ...
                {
                    TableAddLine(table, $"{Viewer.Catalog.GetString("Wind Speed:")} {Viewer.Catalog.GetString("{0:N2} mph", Me.ToMi(pS.TopH(train.PhysicsWindSpeedMpS)))}   " +
                                        $"{Viewer.Catalog.GetString("Wind Direction:")} {Viewer.Catalog.GetString("{0:N2} Deg", train.PhysicsWindDirectionDeg)}   " +
                                        $"{Viewer.Catalog.GetString("Train Direction:")} {Viewer.Catalog.GetString("{0:N2} Deg", train.PhysicsTrainLocoDirectionDeg)}   " +
                                        $"{Viewer.Catalog.GetString("ResWind:")} {Viewer.Catalog.GetString("{0:N2} Deg", train.ResultantWindComponentDeg)}   " +
                                        $"{Viewer.Catalog.GetString("ResSpeed:")} {Viewer.Catalog.GetString("{0:N2} mph", Me.ToMi(pS.TopH(train.WindResultantSpeedMpS)))}");
                    TableAddLine(table);
                }
            }
        ...
        void TextPageDispatcherInfo(TableData table)
        {
            ...
            {
                if (thisTrain.TrainType == Train.TRAINTYPE.PLAYER || (thisTrain.TrainType == Train.TRAINTYPE.REMOTE && Ornp.MultiPlayer.MPManager.IsServer())
        ...
        void TextPageWeather(TableData table)
        {
            ...
            TableAddLabelValue(table, Viewer.Catalog.GetString("Visibility"), Viewer.Catalog.GetString("{0:N0} m", Viewer.Simulator.Weather.FogDistance));
            ...
            TableAddLabelValue(table, Viewer.Catalog.GetString("Intensity"), Viewer.Catalog.GetString("{0:F4} p/s/m^2", Viewer.Simulator.Weather.PricipitationIntensityPPSPM2));
            ...
            TableAddLabelValue(table, Viewer.Catalog.GetString("Wind"), Viewer.Catalog.GetString("{0:F1},{1:F1} m/s", Viewer.Simulator.Weather.WindSpeedMpS.X, Viewer.Simulator.Weather.WindSpeedMpS.Y));
        ...
        void TextPageDebugInfo(TableData table)
        {
            ...
            TableAddLabelValue(table, Viewer.Catalog.GetString("CPU"), Viewer.Catalog.GetString("{0:F0}% ({1})", (Viewer.RenderProcess.Profiler.CPU.SmoothedValue + Viewer.UpdaterProcess.Profiler.CPU.SmoothedValue + Viewer.LoaderProcess.Profiler.CPU.SmoothedValue + Viewer.SoundProcess.Profiler.CPU.SmoothedValue) / ProcessorCount, Viewer.Catalog.GetPluralString("{0} logical processor", "{0} logical processors", ProcessorCount)));
            TableAddLabelValue(table, Viewer.Catalog.GetString("GPU"), Viewer.Catalog.GetString("{0:F0} FPS (50th/95th/99th percentiles {1:F1} / {2:F1} / {3:F1} ms, DirectX feature level >= {4})", Viewer.RenderProcess.FrameRate.SmoothedValue, Viewer.RenderProcess.FrameTime.SmoothedP50 * 1000, Viewer.RenderProcess.FrameTime.SmoothedP95 * 1000, Viewer.RenderProcess.FrameTime.SmoothedP99 * 1000, Viewer.Settings.DirectXFeatureLevel));
            TableAddLabelValue(table, Viewer.Catalog.GetString("Adapter"), Viewer.Catalog.GetString("{0} ({1:F0} MB)", Viewer.AdapterDescription, Viewer.AdapterMemory / 1024 / 1024));
