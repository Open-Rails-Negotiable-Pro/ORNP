...
#pragma warning restore CS1690 // Accessing a member on a field of a marshal-by-reference class may cause a runtime exception
        ...
        private void DrawTrains(Graphics g, PointF scaledA, PointF scaledB)
        {
            ...
            if (F.simulator.TimetableMode)
            {
                ...
                if (F.simulator.PlayerLocomotive.Train is Ornp.Simulation.AIs.AITrain)
                    F.selectedTrainList.Add(F.simulator.PlayerLocomotive.Train as Ornp.Simulation.AIs.AITrain);
        ...
        private void ShowTrainNameAndState(Graphics g, PointF scaledItem, Train t, string trainName)
        {
            if (F.simulator.TimetableMode)
            {
                var tTTrain = t as Ornp.Simulation.Timetables.TTTrain;
                if (tTTrain != null)
                {
