...
using Ornp.Viewer3D.Common;
using Ornp.Viewer3D.Popups;
using ORNP.Common;
using System;
...
namespace Ornp.Viewer3D
{
    ...
    public class SharedTextureManager
    {
                ...
                try
                {
                    ...
                    if (Path.GetExtension(path) == ".dds")
                    {
                        ...
                        // therefore avoiding that routes providing .ace textures show blank global shapes
                        {
                            ...
                            {
                                texture = Ornp.Formats.Msts.AceFile.Texture2DFromFile(GraphicsDevice, aceTexture);
                    ...
                    else if (Path.GetExtension(path) == ".ace")
                    {
                        ...
                        {
                            texture = Ornp.Formats.Msts.AceFile.Texture2DFromFile(GraphicsDevice, path);
                        }
                        else
                        {
                            Texture2D missing()
        ...
        public static Texture2D Get(GraphicsDevice graphicsDevice, string path)
        {
            ...
            if (ext == ".ace")
                return Ornp.Formats.Msts.AceFile.Texture2DFromFile(graphicsDevice, path);
    ...
    public class SharedMaterialManager
    {
        ...
        public SharedMaterialManager(Viewer viewer)
        {
            ...
            if (File.Exists(microtexPath))
            {
                try
                {
                    SceneryShader.OverlayTexture = Ornp.Formats.Msts.AceFile.Texture2DFromFile(viewer.GraphicsDevice, microtexPath);
                }
                catch (InvalidDataException error)
                {
    ...
    public class SceneryMaterial : Material
    {
            ...
            : base(viewer, String.Format("{0}:{1:X}:{2}", texturePath, options, mipMapBias))
        {
            ...
            if (texture.Tag != null && texture.Tag.GetType() == typeof(Ornp.Formats.Msts.AceInfo))
                AceAlphaBits = ((Ornp.Formats.Msts.AceInfo)texture.Tag).AlphaBits;
            else
                AceAlphaBits = 0;
       ...
       public bool LoadDayTexture ()
       {
        ...
        public override void SetState(GraphicsDevice graphicsDevice, Material previousMaterial)
        {
            ...
            var level9_3 = Viewer.Settings.IsDirectXFeatureLevelIncluded(ORNP.Settings.UserSettings.DirectXFeature.Level9_3);
