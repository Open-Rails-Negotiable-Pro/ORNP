...
using Ornp.Simulation.RollingStocks.SubSystems.Controllers;
using Ornp.Viewer3D.Common;
using Ornp.Viewer3D.Popups;
using Ornp.Viewer3D.RollingStock.Subsystems.ETCS;
using ORNP.Common;
using ORNP.Common.Input;
using ORNP.Scripting.Api;
using Event = Ornp.Common.Event;

namespace Ornp.Viewer3D.RollingStock
{
        ...
        public void AttemptToRefillOrUnload()
        {
            ...
            if (distanceToPickupM > match.IntakePoint.WidthM / 2)
            {
                Viewer.Simulator.Confirmer.Message(ConfirmLevel.None, Viewer.Catalog.GetString("Refill: Distance to {0} supply is {1}.",
                    PickupTypeDictionary[(uint)match.Pickup.PickupType], Viewer.Catalog.GetPluralString("{0} meter", "{0} meters", (long)(distanceToPickupM+1f))));
                return;
            }
            if (distanceToPickupM <= match.IntakePoint.WidthM / 2)
