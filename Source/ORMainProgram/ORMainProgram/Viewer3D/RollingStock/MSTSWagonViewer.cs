...
using Ornp.Viewer3D.RollingStock.SubSystems;
using ORNP.Common;
    ...
    public class MSTSWagonViewer : TrainCarViewer
    {
        ...
        private void UpdateAnimation(RenderFrame frame, ElapsedTime elapsedTime)
        {
            ...
            if (MSTSWagon.IsDriveable && MSTSWagon.Simulator.UseAdvancedAdhesion && !MSTSWagon.Simulator.Settings.SimpleControlPhysics)
            {
                ...
                if (Car.EngineType == Ornp.Simulation.RollingStocks.TrainCar.EngineTypes.Steam) // Steam locomotive so set up different speeds for different driver and non-driver wheels
                {
            ...
            else // set values for simple adhesion
            {
                ...
                // Set values of wheel radius - assume that drive wheel and non driven wheel are same sizes
                if (Car.EngineType == Ornp.Simulation.RollingStocks.TrainCar.EngineTypes.Steam) // set values for steam stock
                {
                    AnimationWheelRadiusM = MSTSWagon.WheelRadiusM;
                    AnimationDriveWheelRadiusM = MSTSWagon.DriverWheelRadiusM;
                }
